        ...
        new Elysia({ prefix: "/teams" })
            ...
            .post(
                ...
                async ({ params, body, user }) => {
                    ...
                    // user (P1-3). The alphabet in src/utils/joinCode.ts is
                    // uppercase-only, so normalize the user's input before
                    // comparing — typing "abc23456" instead of "ABC23456"
                    // shouldn't reject a valid code.
                    const suppliedCode =
                        typeof body.code === "string"
                            ? body.code.trim().toUpperCase()
                            : "";
                    if (
                        suppliedCode.length !== 8 ||
                        suppliedCode !== teamResult[0].joinCode
                    ) {
                        throw new AppError(
                            "Invalid join code",
                            403,
