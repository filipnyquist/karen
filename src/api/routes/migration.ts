...
import { migrationLinkEmail, sendEmail } from "../../services/email";
    ...
    .use(
        new Elysia()
            ...
            .post("/send-link", async ({ body }) => {
                ...
                const { subject, html } = migrationLinkEmail(
                    config.baseUrl,
                    token,
                );
                await sendEmail({ to: mapping.oldEmail, subject, html });
