...
export const educationTypes = pgTable("education_types", {
    ...
    // Per-locale fields — populated by the admin UI. The existing
    // `name` / `description` columns stay as the unique-key fallback
    // for rows that haven't been localized yet (and for the seed
    // rows, which originated in Swedish).
    nameSv: text("name_sv"),
    nameEn: text("name_en"),
