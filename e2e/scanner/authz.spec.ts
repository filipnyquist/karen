...
test.describe("Scanner page authz", () => {
    ...
    test("logged-in non-worker is redirected to /event/list", async ({
        page,
    }) => {
        // Login as alice. She's a worker at pub4 (Midsommarpub), but
        // isn't a worker/responsible for pub1/past events. Hit one of
        // those and verify she gets bounced to the event list.
        await login(page, "alice");
        await page.waitForURL("/");

        // Find a past event alice is NOT registered at.
        // pub1 has alice as responsible. pub6 has bob + hanna, no alice.
        // Use pub6 — alice has no registration there.
        ...
        // The server-side authz redirects unauthorized users to /event/list
        // (the scanner landing page no longer exists).
